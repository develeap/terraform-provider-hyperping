...
func TestReservedHeaderName(t *testing.T) {
	...
	tests := []struct {
		...
		{"trailing space Host bypass blocked", types.StringValue("Host "), true},
		{"leading space host bypass blocked", types.StringValue(" host"), true},
		{"internal whitespace blocked", types.StringValue("X-Foo Host"), true},
		{"tab in name blocked", types.StringValue("Host\t"), true},
		{"colon in name blocked", types.StringValue("X-Foo:"), true},
