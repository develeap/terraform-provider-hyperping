...
**Decision -- unused `RequiredWhenProtocolPort` validator**: The `validators_conditional.go` file defines `RequiredWhenProtocolPort` and its tests, but this validator is never applied in the schema. The `ValidateConfig` approach now supersedes it for the port-requires-port-protocol check. However, `RequiredWhenProtocolPort` is a schema-level attribute validator (it cannot express "required when another attribute equals X" without already being set), which is why it was never wired up. We leave this file untouched -- removing dead code is a separate maintenance task outside the scope of IaC usability improvements and would touch test files unnecessarily. The `ValidateConfig` method is the canonical location for this logic going forward.
...
- `http_method`: Fix missing method. Current: `"HTTP method to use. Valid values: \x60GET\x60, \x60POST\x60, \x60PUT\x60, \x60PATCH\x60, \x60DELETE\x60, \x60HEAD\x60. Defaults to \x60GET\x60."` Proposed: `"HTTP method to use. Only valid when protocol is \x60http\x60. Valid values: \x60GET\x60, \x60POST\x60, \x60PUT\x60, \x60PATCH\x60, \x60DELETE\x60, \x60HEAD\x60, \x60OPTIONS\x60. Defaults to \x60GET\x60."` **Rationale**: The existing description is missing `OPTIONS`, which is in `client.AllowedMethods` and accepted by the OneOf validator. Also adds protocol applicability note consistent with other HTTP-only fields.
- `protocol`, `check_frequency`, `expected_status_code`, `alerts_wait`, `escalation_policy`, `project_uuid`: Already good, keep as-is.
